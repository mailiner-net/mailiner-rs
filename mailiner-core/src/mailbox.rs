...
impl MailboxModel {
    ...
    pub async fn sync(&mut self) -> Result<(), Error> {
        ...
        fetched_mailboxes.iter().for_each(|mailbox| self.upsert_mailbox(mailbox));
        ...
        for existing_mailbox in existing_mailboxes {
            if fetched_mailboxes.iter().any(|fetched| match fetched.name {
                ...
                imap_types::mailbox::Mailbox::Other(other) => match other.inner() {
                    ...
                    imap_types::core::AString::String(str) => String::from_utf8_lossy(str.as_ref()) == *existing_mailbox,
                },
            }) {
                continue;
            }
    ...
    pub fn remove_mailbox(&mut self, name: &str) {
        ...
        if self.mailbox_lookup.remove(name).is_none() {
            warn!("Trying to remove mailbox {}, but it does not exist in the model", name);
            return;
        }
        ...
        let mut parent = match name.rfind(self.separator).unwrap_or(0) {
            ...
            pos => {
                ...
                match self.mailbox_lookup.get(parent_name) {
                    ...
                    None => {
                        warn!("Parent mailbox {} (for {}) not found, storage is inconsistent!", parent_name, name);
                        return;
                    }
                }
            }
        ...
        let pos = match parent.read().children.read().iter().position(|child| child.read().name == name) {
            Some(pos) => pos,
            None => {
                warn!("Mailbox {} not found in parent mailbox children, storage is inconsistent!", name);
                return;
            }
        };
    ...
    fn upsert_mailbox(&mut self, mailbox: &Mailbox) {
        self.mailbox_lookup.entry(mailbox.id).and_modify(|current| {
            current.write().total = mailbox.total;
            current.write().unread = mailbox.unread;
        }).or_insert(use_signal(|| mailbox.into()));
    }
}